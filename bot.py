import os, sqlite3, logging, re, asyncio
...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "")   # Render → Environment → BOT_TOKEN set karo
ADMIN_ID  = 8379167923
...
def init_db():
    ...
    c.execute("CREATE INDEX IF NOT EXISTS idx_promo_uses_user  ON promo_uses(user_id, code)")
...
def has_purchased(uid, file_id):
    conn = get_conn()
    p = conn.execute(
        "SELECT id FROM purchases WHERE user_id=? AND file_id=?", (uid, file_id)
    ).fetchone()
...
def has_used_promo(uid, code):
    conn = get_conn()
    r = conn.execute(
        "SELECT id FROM promo_uses WHERE user_id=? AND code=?", (uid, code)
    ).fetchone()
...
async def safe_answer(q, text="", show_alert=False):
    """Safe callback answer — ignore already-answered errors."""
    try:
...
async def safe_edit(q, text, **kwargs):
    """Safe edit — ignore 'message not modified' errors."""
    try:
...
async def check_joined_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_joined:
        ...
        return
    await safe_edit(q, home_text(user), parse_mode="Markdown", reply_markup=main_menu_kb(user.id))
...
async def back_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await safe_answer(q)
    await safe_edit(q, home_text(q.from_user), parse_mode="Markdown", reply_markup=main_menu_kb(q.from_user.id))
...
async def menu_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_joined:
        await safe_edit(q, "⚠️ Pehle channels join karo!", reply_markup=join_kb(not_joined))
        return
    files = get_all_files()
    if not files:
        await safe_edit(
...
async def buy_file_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await safe_answer(q, "❌ Invalid!", show_alert=True); return
    ...
    if not f:
        await safe_answer(q, "❌ File nahi mili!", show_alert=True); return

    # Already purchased — resend
    if has_purchased(uid, f["file_id"]):
        await safe_answer(q, "✅ Yeh file tumhari hai! Neeche bhej raha hoon.", show_alert=True)
        try:
...
async def redeem_code_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not promo:
        await update.message.reply_text(
            "❌ *Invalid Code!*\nYeh code exist nahi karta.",
            parse_mode="Markdown", reply_markup=back_kb
        )
        return ConversationHandler.END
    if promo["used_count"] >= promo["max_uses"]:
        await update.message.reply_text(
            "❌ *Code Expired!*\nIs code ki limit khatam ho gayi.",
            parse_mode="Markdown", reply_markup=back_kb
        )
        return ConversationHandler.END
    if has_used_promo(uid, code):
        await update.message.reply_text(
            "⚠️ *Already Used!*\nTumne yeh code pehle use kar liya.",
            parse_mode="Markdown", reply_markup=back_kb
        )
        return ConversationHandler.END
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.from_user.id != ADMIN_ID:
        await safe_answer(q, "❌ Access Denied!", show_alert=True); return
    await safe_edit(q, "🛠️ *Admin Panel*\n\nKya karna hai? 👇", parse_mode="Markdown", reply_markup=admin_kb())
...
async def adm_add_file_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.update({
        "nf_id":   doc.file_id,
        "nf_name": doc.file_name or "File",
        "nf_type": doc.mime_type or "doc"
    })
    await update.message.reply_text(
...
async def adm_rem_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    files = get_all_files()
    if not files:
        await safe_edit(q, "📭 Koi file nahi hai.",
    ...
    await safe_edit(q, "🗑 *Kaunsi file remove karni hai?*", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
...
async def adm_do_remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if f:
        remove_file_db(fid)
        await safe_edit(
            q, f"✅ *File Removed!*\n\n📄 {f['file_name']} delete ho gayi.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛠️ Admin Panel", callback_data="admin_panel")]])
        )
    else:
        await safe_edit(q, "❌ File nahi mili!")
...
async def adm_crt_promo_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_answer(q)
    if q.from_user.id != ADMIN_ID: return
    await safe_edit(q, "🎟 *Promo Code Create Karo*\n\nCode likhke bhejo (e.g. KING2024)\n\n❌ /cancel", parse_mode="Markdown")
...
async def adm_promo_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if get_promo(code):
        await update.message.reply_text(
            f"❌ Code *{code}* already exist karta hai! Alag daalo.",
            parse_mode="Markdown"
        )
        return CREATE_PROMO_WAIT_CODE
...
async def adm_add_pts_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_answer(q)
    if q.from_user.id != ADMIN_ID: return
    await safe_edit(q, "💰 *User ko Points Add Karo*\n\nUser ka Telegram ID bhejo:\n\n❌ /cancel", parse_mode="Markdown")
...
async def adm_pts_ok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            uid,
            f"🎉 Admin ne tumhe *+{amt} Points* diye!\n💰 Ab files kharido! 🔥",
            parse_mode="Markdown"
        )
    except Exception:
        pass
...
async def adm_bc_ok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg:
        await safe_edit(q, "❌ Message nahi mila!"); return ConversationHandler.END

    users  = get_all_user_ids()
    sent   = failed = 0
    await safe_edit(q, f"📤 Sending to {len(users)} users... Please wait.")
...
async def adm_post_btn_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Link set!\n\nStep 4️⃣: *Kis channel mein post karna hai?* 👇",
        parse_mode="Markdown",
...
async def main():
    if not BOT_TOKEN:
        raise ValueError("❌ BOT_TOKEN environment variable set nahi hai! Render → Environment mein add karo.")
    ...
    logger.info("✅ Sab handlers load ho gaye!")
    logger.info("🔄 Polling mode start ho raha hai...")
    ...
    async with app:
        ...
        await app.start()
        logger.info("✅ Bot chal raha hai! Rok ne ke liye Ctrl+C dabao.")
        await asyncio.Event().wait()   # forever until killed
